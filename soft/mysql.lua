...
local function _parse_row_data_packet(data, cols, compact, jsonColSet)
    local pos = 1
    local ncols = #cols
    local row
    if compact then
    ...
    for i = 1, ncols do
        ...
        if jsonColSet and jsonColSet[name] then
            if value ~= "" then
...
local function read_result(self, jsonColSet, est_nrows)
    ...
    while true do
        ...
        local row = _parse_row_data_packet(packet, cols, compact, jsonColSet)
        i = i + 1
        rows[i] = row
    end
...
function _M.query(self, query, jsonColSet, est_nrows)
    local bytes, err = send_query(self, query)
    if not bytes then
        return nil, "failed to send query: " .. err
    end

    return read_result(self, jsonColSet, est_nrows)
end
